...
public class AdvanceSudokuBoard extends SudokuBoard {
   ...
   public int[] getPossibleValues(SudokuCoords sudokuCoords) {
      List<Integer> possibleValues = IntStream.iterate(1, k -> k + 1).limit(9).mapToObj(k -> k).collect(Collectors.toList());
      ...
      return possibleValues.stream().filter(pv -> notPossibleValues.stream().noneMatch(npv -> npv.equals(pv))).mapToInt(k -> k).toArray();
   }


   public AdvanceSudokuBoard cloneBoard() {
      return new AdvanceSudokuBoard((int[][]) clone());
   }
}
